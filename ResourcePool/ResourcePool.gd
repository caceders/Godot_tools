...
## The start/stop replenish/depletion methids.
...
@export var base_growth_rest_period: float = 2
## How long should the delays mentioned above be
@export var base_decay_rest_period: float = 2

# Externaly accessd amount property ##DUMB, CHANGE THE PRIVATE PROPERTY BELOW THEN >:C##
var amount: float = 1:
	get: return get_amount()

## Variables used to keep track of how many active calls to rest in decay and growth
var _calls_to_rest_growth: int = 0
var _calls_to_rest_decay: int = 0

var _growth_is_resting: bool = false
var _decay_is_resting: bool = false
...
func rest_growth(time: float = base_growth_rest_period) -> void:
	# Begin rest
	_calls_to_rest_growth += 1
	_growth_is_resting = true
	...
	# End rest
	_calls_to_rest_growth -= 1
	if _calls_to_rest_growth == 0:
		_growth_is_resting = false


## Pauses decay of the resource pool for the given amount of seconds
func rest_decay(time: float = base_decay_rest_period) -> void:
	# Begin rest
	_calls_to_rest_decay += 1
	_decay_is_resting = true
	...
	# End rest
	_calls_to_rest_decay -= 1
	if _calls_to_rest_decay == 0:
		_decay_is_resting = false
